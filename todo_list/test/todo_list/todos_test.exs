defmodule TodoList.TodosTest do
  ...
  describe "items" do
    ...
    @valid_attrs %{complete_at: "2010-04-17T14:00:00Z", content: "some content"}
    @update_attrs %{complete_at: "2011-05-18T15:01:01Z", content: "some updated content"}
    @invalid_attrs %{complete_at: nil, content: nil}
    ...
    test "create_item/1 with valid data creates a item" do
      assert {:ok, %Item{} = item} = Todos.create_item(@valid_attrs)
      assert item.complete_at == DateTime.from_naive!(~N[2010-04-17T14:00:00Z], "Etc/UTC")
    ...
    test "update_item/2 with valid data updates the item" do
      item = item_fixture()
      assert {:ok, %Item{} = item} = Todos.update_item(item, @update_attrs)
      assert item.complete_at == DateTime.from_naive!(~N[2011-05-18T15:01:01Z], "Etc/UTC")
